...
function App() {
  ...
  async function analize(el) {
    ...
    setLoading(true)
    setKondisi(null)
    const request = await fetch('http://127.0.0.1:8000/analyze', {
      method: 'POST',
      body: formData,
  ...
  return (
    <main className='container flex-row mx-auto mt-5 rounded-sm h-96 mb-20'>
      ...
      <section className='mx-auto mt-5 sm:max-w-[500px] md:w-[500px] px-3'>
        <form action="" onSubmit={analize.bind(this)} className='shadow-lg rounded-lg border px-5 py-6 grid grid-rows-4'>
